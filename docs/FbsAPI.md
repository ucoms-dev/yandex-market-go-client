...
Other parameters are passed through a pointer to an `apiGetOrdersRequest` struct via the builder pattern.
...
| **status** | [**[]OrderStatusType**](OrderStatusType.md) | Статус заказа:<br>• `CANCELLED` — заказ отменен.<br>• `DELIVERED` — заказ получен покупателем.<br>• `DELIVERY` — заказ передан в службу доставки.<br>• `PICKUP` — заказ доставлен в пункт самовывоза.<br>• `PROCESSING` — заказ находится в обработке.<br>• `UNPAID` — заказ оформлен, но ещё не оплачен.<br>Могут возвращаться и другие значения — обрабатывать их не требуется. | |
| **substatus** | [**[]OrderSubstatusType**](OrderSubstatusType.md) | Этап обработки заказа (`PROCESSING`) или причина отмены (`CANCELLED`).<br><br>Для `PROCESSING`:<br>• `STARTED`<br>• `READY_TO_SHIP`<br>• `SHIPPED`<br><br>Для `CANCELLED`:<br>• `RESERVATION_EXPIRED`<br>• `USER_NOT_PAID`<br>• `USER_UNREACHABLE` (см. условия в описании)<br>• `USER_CHANGED_MIND`<br>• `USER_REFUSED_DELIVERY`<br>• `USER_REFUSED_PRODUCT`<br>• `SHOP_FAILED`<br>• `USER_REFUSED_QUALITY`<br>• `REPLACING_ORDER`<br>• `PROCESSING_EXPIRED`<br>• `PICKUP_EXPIRED`<br>• `DELIVERY_SERVICE_UNDELIVERED`<br>• `CANCELLED_COURIER_NOT_FOUND`<br>• `USER_WANTS_TO_CHANGE_DELIVERY_DATE`<br>• `RESERVATION_FAILED`<br>Другие значения возможны, но их обработка не требуется. | |
| **fromDate** | **string** | Начальная дата фильтрации по дате оформления (формат `ДД-ММ-ГГГГ`).<br>Максимальный диапазон — 30 дней. По умолчанию: 30 дней назад. | |
| **toDate** | **string** | Конечная дата фильтрации по дате оформления (формат `ДД-ММ-ГГГГ`).<br>Не включается в интервал. По умолчанию: текущая дата.<br>Если диапазон меньше суток — `toDate = fromDate + 1 день`. | |
| **supplierShipmentDateFrom** | **string** | Начальная дата фильтрации по дате отгрузки (формат `ДД-ММ-ГГГГ`). Включается в интервал. | |
| **supplierShipmentDateTo** | **string** | Конечная дата фильтрации по дате отгрузки (формат `ДД-ММ-ГГГГ`).<br>Не включается в интервал. Если диапазон меньше суток — `toDate = fromDate + 1 день`. | |
| **updatedAtFrom** | **time.Time** | Начальная дата фильтрации по дате и времени обновления.<br>Формат: ISO 8601 с UTC смещением (например, `2017-11-21T00:42:42+03:00`).<br>Максимум 30 дней. Включительно. | |
| **updatedAtTo** | **time.Time** | Конечная дата фильтрации по дате и времени обновления.<br>Формат: ISO 8601 с UTC смещением. Не включается в интервал. | |
| **dispatchType** | [**OrderDeliveryDispatchType**](OrderDeliveryDispatchType.md) | Способ отгрузки. | |
| **fake** | **bool** | Фильтрация по типу заказа:<br>• `false` — настоящий заказ.<br>• `true` — [тестовый](../../concepts/sandbox.md) заказ. | По умолчанию: `false` |
| **hasCis** | **bool** | Возвращать только заказы с маркированными товарами (`Честный ЗНАК`, `ASL BELGISI`). | По умолчанию: `false` |
| **onlyWaitingForCancellationApprove** | **bool** | **Только для DBS.**<br>Фильтр по заказам, ожидающим подтверждения отмены от продавца.<br>Используется в статусах `DELIVERY` и `PICKUP`. Подробнее — [здесь](../../reference/orders/acceptOrderCancellation). | По умолчанию: `false` |
| **onlyEstimatedDelivery** | **bool** | Фильтр по заказам с длительной доставкой (31–60 дней):<br>• `true` — только неподтвержденные даты доставки.<br>• `false` — без фильтрации. | По умолчанию: `false` |
| **buyerType** | [**OrderBuyerType**](OrderBuyerType.md) | Фильтрация по типу покупателя. | |
| **page** | **int32** | Номер страницы.<br>Используется с `pageSize`. Игнорируется, если задан `pageToken` или `limit`. Подробнее — [здесь](../../concepts/pagination.md). | По умолчанию: `1` |
| **pageSize** | **int32** | Размер страницы. Используется с `page`. Игнорируется, если задан `pageToken` или `limit`. | |
| **pageToken** | **string** | Идентификатор страницы. Используйте `nextPageToken` из предыдущего ответа. Заменяет `page` и `pageSize`. | |
