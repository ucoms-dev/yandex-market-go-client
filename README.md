...
\n+## De-duplication Audit (2025-08-28)

- Scope: searched for duplicate struct/type definitions across `model_*.go` and `api/*.go`.
- Result: no duplicate struct/type names found; `go build ./...` succeeds.
- Notes: per-service request builder structs (e.g., `FbsAddOffersToArchiveRequest`, `FbyAddOffersToArchiveRequest`) are intentionally distinct and reference a single body model (e.g., `AddOffersToArchiveRequest`). These are generated by OpenAPI and not duplicates by name.
- How to re-check:
  - List duplicate struct names: `rg -n "^type\\s+([A-Za-z0-9_]+)\\s+struct\\b" | sed -E 's/^.*type\\s+([A-Za-z0-9_]+)\\s+struct.*/\\1/' | sort | uniq -d`
  - Build verification: `go build ./...`


## JSON Decoding: Unknown Fields

- Change (2025-08-28): relaxed JSON decoding across all models to ignore unknown fields returned by the API.
- Why: to prevent runtime errors like `json: unknown field "logisticPointId"` when the server includes additional properties not yet reflected in this client.
- How: removed `decoder.DisallowUnknownFields()` from model `UnmarshalJSON` implementations and from `newStrictDecoder` in `utils.go`.
- Impact: decoding no longer fails on extra fields; known fields are still parsed as before.
