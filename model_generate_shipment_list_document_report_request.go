...
type GenerateShipmentListDocumentReportRequest struct {
	...
	// Идентификатор отгрузки.  В запросе обязательно передавайте `shipmentId` или `orderIds`.
	ShipmentId *int64 `json:"shipmentId,omitempty"`
	// Фильтр по идентификаторам заказа в отгрузке.  В запросе обязательно передавайте `shipmentId` или `orderIds`.
...
func (v *NullableGenerateShipmentListDocumentReportRequest) Unset() {
	var zero *GenerateShipmentListDocumentReportRequest
	v.value = zero
	v.isSet = false
}
